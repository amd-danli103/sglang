...
class TestHiSparseUnifiedPool(unittest.TestCase):
    ...
    def test_views_alias_unified_compressed_region(self):
        ...
        for c, local_id in enumerate(c4_ids):
            ...
            self.assertTrue(
                torch.all(unified.kv_buffer[local_id][:swa_pages] == 0)
            )
    ...
    def test_page_size_one_alloc_free_mapping(self):
        ...
        mapping = torch.cat(
            [
                torch.zeros(logical_size + pool.page_size, dtype=torch.int64, device="cuda"),
        ...
        self.assertTrue(
            torch.equal(translated.long(), hisparse_indices.long())
        )
        self.assertTrue(
