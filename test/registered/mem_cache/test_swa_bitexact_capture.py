...
class TestSwaStrideCapture(unittest.TestCase):
    """Capture keeps one window every ``stride`` pages PLUS the true
...
class TestSwaHostSizingByStride(unittest.TestCase):
    """SWA host pool is sized as ceil(full_host_pages / stride) + tail,
...
class TestSwaStrideArg(unittest.TestCase):
    """--hicache-swa-offload-page-stride parses and defaults to 1.
...
class TestHostPoolUpperBoundGuard(unittest.TestCase):
    """Startup budget guard for the SWA offload host pool.
...
class TestSwaHostHardLimitFromDram(unittest.TestCase):
    """DRAM-aware: the hard fail-fast ceiling is derived from real
