"""Unit test for c4 / c4-indexer state riding on strict bit-exact prefix reuse.

Exercises the state-riding bind/promote/restore helpers and the state half of the
strict reuse gate with faked collaborators and real CPU tensors -- no GPU or
model required:
...
register_amd_ci(est_time=35, suite="stage-b-test-1-gpu-small-amd")
...
class TestDecodeSourceCapture(unittest.TestCase):
    """Decode-source capture: the boundary group state crossed DURING
