...
def _swa_scatter_two_pool_kernel(
    ...
    nope = tl.load(
        kv_nope_ptr + row * D_NOPE + nope_offs, mask=nope_mask, other=0
    )
    tl.store(
        unified_nope_ptr + loc * D_NOPE + nope_offs, nope, mask=nope_mask
    )

    rope_offs = tl.arange(0, BLOCK_ROPE)
    rope_mask = rope_offs < D_ROPE
    rope = tl.load(
        kv_rope_ptr + row * D_ROPE + rope_offs, mask=rope_mask, other=0.0
    )
    tl.store(
        unified_rope_ptr + loc * D_ROPE + rope_offs, rope, mask=rope_mask
    )
