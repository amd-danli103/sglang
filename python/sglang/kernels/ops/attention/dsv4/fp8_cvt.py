...
def cvt_fp8_e4m3(src: torch.Tensor) -> torch.Tensor:
    ...
    :param src: contiguous 1D fp32 CUDA/HIP tensor, even length (the conversion
                runs two values at a time).
    :returns: uint8 tensor of the same length holding the raw e4m3 bytes.
    :raises RuntimeError: if the length is zero or odd, or a tensor does not match.
    """
