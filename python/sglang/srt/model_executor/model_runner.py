    ...
    def forward(
        ...
        if not self.is_draft_worker and forward_batch.forward_mode.is_decode():
            ...
            if _swa_decode_capture is not None:
                ...
                # overlap scheduler is the worker's forward_stream -- the same
                # stream that ran this decode forward and that will run the next
                # one. So the ring read is same-stream-ordered between the two ring
                # writes whether overlap is on or off. Cross-stream consumers of the
                # captured host page (restore H2D, L3 write-through, device-landing
                # check) are instead ordered by the capture-completion event that
            ...
            # stream and event contract as the SWA decode capture above. No-op
            # unless the strict state riding pools are wired.
